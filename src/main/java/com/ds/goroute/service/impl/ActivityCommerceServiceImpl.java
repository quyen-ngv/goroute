...
public class ActivityCommerceServiceImpl implements ActivityCommerceService {
 ...
 /**
  * One product by id, for a caller who has its id and nothing else — a deep link, a push
  * notification, or the app opening a listing that is not in the page it loaded. Mirrors
  * {@code partnerGetHotel}: the resource decides the organisation, so the caller does not
  * have to know it.
  *
  * <p>Deliberately not {@code displayed()}: a partner edits and sells in the currency they
  * set, and converting it for them is how a price gets saved back in the wrong one.
  */
 public MarketplaceActivityResponse partnerGet(UUID actor,UUID id){MarketplaceActivityProduct p=product(id);authorization.requireResourcePermission(p.getOrganizationId(),actor,"ACTIVITY",p.getId(),"ACTIVITY_READ");return productResponse(p);}
 ...
 public List<MarketplaceActivityResponse> adminProducts(String q,java.util.List<String> s,java.util.List<UUID> areas,String sort,boolean descending,int p,int z){Page r=page(p,z);return repository.findProductsAdmin(clean(q),s,areas,sort,descending,r.limit,r.offset).stream().map(this::productResponse).toList();}
 ...
 public List<ActivityOrderResponse> adminOrders(String q,java.util.List<String> s,java.util.List<String> pay,String sort,boolean descending,int p,int z){Page r=page(p,z);return repository.findOrdersAdmin(clean(q),s,pay,sort,descending,r.limit,r.offset).stream().map(this::orderResponse).toList();}
