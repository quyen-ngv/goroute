...
public class HotelMarketplaceServiceImpl implements HotelMarketplaceService {
    ...
    public RatePlanResponse partnerCreateRate(UUID actor, UUID roomId, UpsertRatePlanRequest request) {
        ...
        RatePlan rate=RatePlan.builder().id(UUID.randomUUID()).roomTypeId(roomId).code(normalizeCode(request.getCode())).name(request.getName().trim())
                .currency(request.getCurrency().toUpperCase()).basePrice(request.getBasePrice()).mealPlan(enumNameOrDefault(request.getMealPlan(),"ROOM_ONLY"))
                ...
                .minStay(request.getMinStay()).maxStay(request.getMaxStay()).status(enumNameOrDefault(request.getStatus(),MarketplaceAvailabilityStatus.ENABLED))
    ...
    public RatePlanResponse partnerUpdateRate(UUID actor, UUID rateId, UpsertRatePlanRequest request) {
        ...
        rate.setBasePrice(request.getBasePrice());rate.setMealPlan(enumNameOrDefault(request.getMealPlan(),rate.getMealPlan()));
        ...
        rate.setMinStay(request.getMinStay());rate.setMaxStay(request.getMaxStay());rate.setStatus(enumNameOrDefault(request.getStatus(),rate.getStatus()));
    ...
    public HotelBookingResponse createBooking(UUID userId,CreateHotelBookingRequest request){
        ...
        HotelBooking booking=HotelBooking.builder().id(UUID.randomUUID()).bookingCode(bookingCode()).userId(userId).organizationId(hotel.getOrganizationId())
                ...
                .discountAmount(BigDecimal.ZERO).totalAmount(subtotal).bookingStatus(MarketplaceBookingStatus.PENDING_PAYMENT.name()).paymentStatus(MarketplacePaymentStatus.UNPAID.name()).source("GOROUTE")
    ...
    private HotelBookingResponse transitionBooking(HotelBooking b,MarketplaceBookingStatus target,String reason,Long requestedVersion,UUID actor,String actorType){
        ...
        else if(target.releasesInventory()){boolean reserved=MarketplaceBookingStatus.PENDING_PAYMENT.name().equals(b.getBookingStatus());int n=repository.releaseInventory(item.getRoomTypeId(),b.getCheckInDate(),b.getCheckOutDate(),item.getQuantity(),reserved,actor,now);if(n!=nights)throw conflict("Allocated inventory is inconsistent");}
    ...
    private boolean organizationBookable(UUID organizationId){return organizationRepository.findById(organizationId)
            .map(org->OrganizationOperationalStatus.ENABLED.name().equals(org.getOperationalStatus())&&!OrganizationVerificationStatus.SUSPENDED.name().equals(org.getVerificationStatus()))
            .orElse(false);}
