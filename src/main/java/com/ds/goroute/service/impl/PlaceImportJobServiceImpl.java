...
public class PlaceImportJobServiceImpl implements PlaceImportJobService {
    ...
    private PlaceImportJobResponse toResponse(PlaceImportJob job, List<PlaceImportJobItem> items) {
        return PlaceImportJobResponse.builder()
                ...
                .currentRegionCode(job.getCurrentRegionCode())
                .currentRegionName(job.getCurrentRegionName())
                .processedCount(job.getProcessedCount())
                .eligibleCount(job.getEligibleCount())
                .importedCount(job.getImportedCount())
                .rejectedScoreCount(job.getRejectedScoreCount())
                .insufficientPhotoCount(job.getInsufficientPhotoCount())
                .cancelRequested(job.getCancelRequested())
                .selectedReviews(job.getSelectedReviews())
                .minReviewCount(job.getMinReviewCount())
                .minAdjustedRating(job.getMinAdjustedRating())
    ...
    private PlaceImportJobItemResponse toItemResponse(PlaceImportJobItem item) {
        return PlaceImportJobItemResponse.builder()
                ...
                .scrapedReviewCount(item.getScrapedReviewCount())
                ...
                .outcomeReason(item.getOutcomeReason())
