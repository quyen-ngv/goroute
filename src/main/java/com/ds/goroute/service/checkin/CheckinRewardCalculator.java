...
public class CheckinRewardCalculator {
    ...
    public record Reward(int points, String reason) {
    ...
    public Reward calculate(UserCheckin checkin, int pointsAlreadyEarnedToday) {
        if (!config.getBoolean(BusinessConfigKey.CHECKIN_REWARD_ENABLED)) {
            return new Reward(0, "Rewards are currently switched off");
        }
        ...
        if (checkin.getVerificationStatus() != CheckinVerificationStatus.VERIFIED) {
            ...
            reasons.add("location could not be confirmed");
        }
        ...
        if (points > remaining) {
            points = remaining;
            reasons.add("daily limit of " + dailyCap + " points reached");
        }

        return new Reward(points, String.join("; ", reasons));
    }
}
